...
public class SearchresultController extends AbstractBaseController
{
   ...
   */
  @Data
  class ProcessedFilterBundle {
    final private List<FilterViewModel> FilterViewModelList;
    final private Boolean hasActiveFilters; 
  }
      ...
      Model model) throws SystemException
  {
    ...
    if (CollectionUtils.isNotEmpty(editorOrganizationList))
    {
      ...
      // at this point we know: This user can edit some datasets. So this user can create new datasets.
      newDatasetUrlString =
    ...
    String clearDateFromUrl =
        urlbuilder.createUrl(response, new String[] {QueryParamNames.PARAM_FROM}).toString();
    String clearDateUntilUrl =
        urlbuilder.createUrl(response, new String[] {QueryParamNames.PARAM_UNTIL}).toString();
    
    // prepare flag for hiding filter options not usable on liferay-results (basically: check if ckan types are there)
    ...
    if(Arrays.asList(SearchConsts.CKAN_TYPES_ALL).contains(preparm.getType())) {
      for(FilterViewModel filterView : filterViewModelMap.get("type")) {
        if(Arrays.asList(SearchConsts.CKAN_TYPES).contains(filterView.getName()) && filterView.getDocCount() > 0) {
    ...
    if(!result.getSuggestions().isEmpty()) {
      List<SuggestionOption> suggestions = new ArrayList<SuggestionOption>(result.getSuggestions());
    ...
    SearchResultsViewModel searchResultsViewModel = SearchResultsViewModel.builder()
        ...
        .dateFrom(urlbuilder.getParam(QueryParamNames.PARAM_FROM))
        .dateUntil(urlbuilder.getParam(QueryParamNames.PARAM_UNTIL))
  ...
  {
    HashSet<String> usedPhrases = null;
  ...
  /**
   * Iterate all available filters and generate actionURLs to toggle each filter option.
   * The active / inactive flag and the actionURL are directly set for each filter in filterMap.
      ...
      PortletURL filterActionUrl, Locale locale, long totalHits)
  {
    ...
    // this will be modified by removing or adding an option to create the actionURLs for each filter-toggle.
    ...
    for (Entry<String, FilterViewListModel> filterGroup : filterViewModelMap.entrySet())
    {
      ...
      for (FilterViewModel filter : filterGroup.getValue())
      {
        ...
        // i18n filter name
        filter.setDisplayName(translateFilterName(filter.getName(), key, locale));
      ...
      // if there is a filter, but it is not part of the aggregation from server - include it anyways
      for(String remainingItem : groupList) {
        if(StringUtils.isBlank(remainingItem)) { // no empty / whitespace-items
          continue;
        }
  ...
  private String translateFilterName(String filterName, String filterType, Locale locale)
  {
    ...
    if(StringUtils.equals(filterType, SearchConsts.FACET_ISOPEN)) {
