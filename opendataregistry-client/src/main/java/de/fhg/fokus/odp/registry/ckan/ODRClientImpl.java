...
public class ODRClientImpl implements ODRClient {
	...
	public User findUser(String name) {
		...
		// Der Parameter “checker” NUR bei dem Wert true enthält datasets
		// ausschließlich die Namen der Datensätze. Sofern der Parameter
		// nicht gesetzt wird, verhält sich user_show wie bisher.

		userName.put("checker", true);
		UserImpl impl = null;
			...
			if (response.getStatus() == Response.Status.OK.getStatusCode()) {
				...
				if (packages != null && packages.isArray()) {
					for (JsonNode package_ : packages) {
						/*
						 * JsonNode packageName = package_.get("name"); if
						 * (packageName != null && packageName.isTextual()) {
						 * impl.getDatasets().add(packageName.getTextValue()); }
						 */
						if (package_ != null && package_.isTextual()) {
							impl.getDatasets().add(package_.getTextValue());
						}
					}
				}
			} else {
