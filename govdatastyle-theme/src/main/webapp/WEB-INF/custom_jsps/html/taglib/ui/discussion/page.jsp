		...
		<aui:form action="<%= formAction %>" method="post" name="<%= formName %>">
			...
			<c:if test="<%= !hideControls && MBDiscussionPermission.contains(permissionChecker, company.getCompanyId(), scopeGroupId, permissionClassName, permissionClassPK, userId, ActionKeys.ADD_DISCUSSION) %>">
				<aui:fieldset cssClass="add-comment" id='<%= randomNamespace + "messageScroll0" %>'>
					...
					<div id="<%= randomNamespace %>postReplyForm<%= i %>" class="lfr-discussion-form-reply" style="display: none;">
						<%
						String postReplyButtonLabel = LanguageUtil.get(pageContext, "reply");
						String textareaPlaceholder = LanguageUtil.get(pageContext, "od.comment.new.name");
						...
						<aui:input id='<%= randomNamespace + "postReplyBody" + i %>' label="comment" placeholder="<%= textareaPlaceholder %>" name='<%= "postReplyBody" + i %>' type="textarea" wrap="soft" wrapperCssClass="lfr-textarea-container" />
			...
			<c:if test="<%= messagesCount > 1 %>">
				...
				<aui:row>
					...
					<%
					if (messages != null) {
						messages = ListUtil.sort(messages, new MessageCreateDateComparator(false)); // sort descending
                  ...
                  <%
                  User commentUser = UserLocalServiceUtil.fetchUser(message.getUserId());
                  %>
                  <c:choose>
                    <c:when test="<%= (commentUser != null && commentUser.isActive()) %>">
                    ...
                    <c:otherwise>
                      ...
                      />
								...
								<aui:col cssClass="lfr-discussion-body" width="75">
									...
									<blockquote class="lfr-discussion-message">
							...
							<aui:row cssClass="lfr-discussion-form-container" fluid="<%= true %>">
								<div class="lfr-discussion-form lfr-discussion-form-reply span12" id="<%= randomNamespace %>postReplyForm<%= i %>" style='<%= "display: none; max-width: " + ModelHintsConstants.TEXTAREA_DISPLAY_WIDTH + "px;" %>'>
									...
									<aui:input id='<%= randomNamespace + "postReplyBody" + i %>' label="comment" name='<%= "postReplyBody" + i %>' style='<%= "height: " + ModelHintsConstants.TEXTAREA_DISPLAY_HEIGHT + "px;" %>' type="textarea" placeholder='<%= LanguageUtil.get(pageContext, "od.comment.new.name") %>' wrap="soft" />
